...
indicators = ["T", "TM", "Tm", "PP", "V", "RA", "SN", "TS", "FG"];
...
adjR2 = 1 - ((n-1)/(n-10))*(sum((GR - y).^2)/sum((GR - mean(GR)).^2));
R2 = 1-(sum((GR - y).^2)/sum((GR - mean(GR)).^2));
se2 = (1/(n-10))*(sum((GR - y).^2));
...
[b_2,se_2,p_2,inmodel,stats,nextstep,hist]=stepwisefit(data.', GR);
...
for i=1:length(k)
    ...
    if(sum(sqrt(GR-yfittemp)) < sum(sqrt(GR-yfitRR)))
        bRR = bRRv(:,i);    
        yfitRR = yfittemp;
        idx = i;
    end
...
adjR2_3 = 1 - ((n-1)/(n-10))*(sum((GR - yfitRR).^2)/sum((GR - mean(GR)).^2));
R2_3 = 1-(sum((GR - yfitRR).^2)/sum((GR - mean(GR)).^2));
se2_3 = (1/(n-10))*(sum((GR - yfitRR).^2));
